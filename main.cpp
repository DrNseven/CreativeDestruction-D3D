...
HRESULT APIENTRY EndScene_hook(IDirect3DDevice9* pDevice)
{
	...
	if (info)
	{
		...
		ImGui::Text("Hack loaded. Press INSERT for menu, ESC & MOUSE to navigate");
		ImGui::End();
	...
	if (ShowMenu)
	{
		...
		const char* ChamTypes_Options[] = { "1", "2", "3" };

		ImGui::Begin("Hack Menu");
		ImGui::Checkbox("Wallhack", &wallhack);
		ImGui::Checkbox("Esp", &esp);
		//ImGui::PushItemWidth(-1);
		//ImGui::Text("Cham Type");
		//ImGui::SameLine();
		//ImGui::Combo("##ChamType", (int*)&chamtype, ChamType_Options, IM_ARRAYSIZE(ChamType_Options));
		//ImGui::PopItemWidth();
		...
		if (ModelEspInfo.size() != NULL)
		{
			for (unsigned int i = 0; i < ModelEspInfo.size(); i++)
			{
				if (ModelEspInfo[i].pOutX > 1.0f && ModelEspInfo[i].pOutY > 1.0f && ModelEspInfo[i].RealDistance > 13.0f)
				{
					...
					if(ModelEspInfo[i].RealDistance > 6.0f && ModelEspInfo[i].RealDistance < 200.0f)
	...
	if (ModelEspInfo.size() != NULL)
	{
		...
		if (BestTarget != -1 && ModelEspInfo[BestTarget].RealDistance > 10.0f)
		{
