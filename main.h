...
void AddModels(LPDIRECT3DDEVICE9 Device, UINT sr)
{
	...
	//xx = ((position.x / position.w) * (Viewport.Width / 2)) + Viewport.X + (Viewport.Width / 2); //scope is both /2 & /1.07
	xx = ((position.x / position.w) * (Viewport.Width / 1.07f)) + Viewport.X + (Viewport.Width / 1.07f); //107
	yy = Viewport.Y + (Viewport.Height / 2) - ((position.y / position.w) * (Viewport.Height / 2));

	ModelEspInfo_t pModelEspInfo;
	pModelEspInfo = { static_cast<float>(xx), static_cast<float>(yy+(Viewport.Height/40.0f)), static_cast<float>(position.w*0.4f) }; //-25
